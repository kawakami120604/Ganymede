...
int main( void ){ 
	...
	//岩石層の定数
	double r_rock = 1.78e6;       //岩石層の表面半径[m]
	...
	for(i=0; i<78; i++)	{
		...
		if (i*dr < (r_rock-r_core)-i*dr) 
			l[i] = i*dr;
		else
			l[i] = (r_rock-r_core)-i*dr;
	}
	// fclose(file_g);
	...
	for(j = 0; j < 460001; j++){
		...
		if(j==0){
			...
			//断熱温度勾配の計算
			f_ad = -k_core * alpha_core * T[0] * g[0] / Cp_core;
			//CMBでの熱流量の計算
			f_cmb = k_core * (T[1]-T[0]) / dr;
			//コア内の半径rでの総熱流量
			f_core_tot = - rho_core * Cp_core * r_core * (T[0] - T_old[0])/dt /3;
			f_core_conv = f_core_tot - f_ad[0];
			//平均流速を計算
			...
			// エルザッサー数
			lambda = sigma_core * pow(B,2.0) / ( 2.0 * rho_core * omega );
			
			//境界条件を適用
			T[0] = 3 * f_cmb * dt / (Cp_rock * rho_core * r_core) + T_old[0];				//内側の境界条件(∵ f_cmb(j-1) * S_c * dt = Cp_rock * rho_c * V_c * (T_old[0](j-1)-T_old[0](j)) )
			T_old[77] = 278.6 ;			                                                	    //外側の境界条件
			//温度の計算
			for(i=1; i<77; i++)	{
				f_ad[i] = 
				T[i] = dt/dr/dr*a_rock * ((1.0+1.0/(i+100))*T_old[i+1] - 2.0*T_old[i] + (1.0-1.0/(i+100))*T_old[i-1]) + T_old[i] + q_all/Cp_rock;
		...
		//時間変化を出力
		printf("%d,%f,%f,%f,%f,%f,%f\n", j, T_old[0], f_core_tot, f_core_conv, R_ml, R_mac, lambda);
		fprintf(file_j, "%d,%f,%f,%f,%f,%f,%f\n", j, T_old[0], f_ad[0], f_cmb, R_ml, R_mac, lambda); 
	}
	fclose(file_j);
